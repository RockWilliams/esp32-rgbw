...
IPAddress local_IP(6, 10, 10, 237);
...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data)
{
  ...
  for (int i = 71; i < 110; i++)// for (int i = 0; i < length / 4; i++)
  {
    leds[i-71] = CRGBW(data[i * 4 + 1], data[i * 4], data[i * 4 + 2], data[i * 4 + 3]);   // data[i * 4] is the first of the 4 channels per led and so on, Have to swap the first and second data value
  }
