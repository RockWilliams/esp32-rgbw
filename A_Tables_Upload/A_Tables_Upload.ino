...
IPAddress local_IP(6, 10, 10, 211);
...
//  }
  ...
  for (int i = 0; i < 60; i++)// for (int i = 0; i < length / 4; i++)
  {
//    int led = i + (universe - startUniverse) * (previousDataLength / 4);
//    if (led < numLeds)
      leds[i] = CRGBW(data[i * 4 + 1], data[i * 4], data[i * 4 + 2], data[i * 4 + 3]); //first and second value for data[] are swapped
  }
  // data[i * 4] is the first of the 4 channels per led and so on
